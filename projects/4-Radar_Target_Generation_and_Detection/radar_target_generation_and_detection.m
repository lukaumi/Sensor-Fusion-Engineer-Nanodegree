...
% Calculate the Bandwidth (B), Chirp Time (Tchirp) and
% Slope (slope) of the FMCW chirp using the requirements above.
B = c / (2 * range_res);
...
% Timestamp for running the displacement scenario
% for every sample on each chirp
...
% Nr and Nd here would also define the size of
% Range and Doppler FFT respectively.
Mix_range = reshape(Mix, [Nr,Nd]);
% DONE

% *%TODO*:
% Run the FFT on the beat signal along the range bins dimension (Nr) and
% normalize.
sig_fft = fft(Mix_range, Nr)./Nr;
% DONE
...
% *%TODO*:
% Output of FFT is double sided signal, but we are interested
% in only one side of the spectrum.
...
Tr = 10;  % range training cells
Td = 8;  % distance training cells
% DONE

% *%TODO*:
% Select the number of Guard Cells in both dimensions
% around the Cell under test (CUT) for accurate estimation.
Gr = 4;  % range guard cells
Gd = 4;  % distance guard cells
% DONE
...
% If the CUT level > threshold assignit a value of 1, else equate it to 0.

% Use RDM[x,y] as the matrix from the output of 2D FFT for implementing
% CFAR

% https://knowledge.udacity.com/questions/564543

Tcells_num = (2 * Tr + 2 * Gr + 1) * (2 * Td + 2 * Gd + 1) - (2 * Gr + 1) * (2 * Gd + 1);
...
for i = Tr + Gr + 1 : Nr/2 - (Gr + Tr)
    for j = Td + Gd + 1 : Nd - (Gd + Td)
        ...
        threshold = pow2db(noise_level / Tcells_num) + offset;
...
% https://www.mathworks.com/matlabcentral/answers/321854-assigning-binary-values-1-0-to-a-data-set-depending-on-the-values-within
RDM(RDM~=0 & RDM~=1) = 0;
% DONE

% *%TODO*:
% Display the CFAR output using the Surf function like we did for
